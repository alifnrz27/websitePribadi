<x-guest-layout>
    ...
    <section id="project" class="pt-36 pb-32 bg-slate-100 dark:bg-dark">
        <div class="container">
            <div class="w-full px-4">
                <div class="max-w-xl mx-auto text-center mb-16">
                    <h4 class="font-semibold text-lg text-primary mb-2">Project</h4>
                    <h2 class="font-bold text-dark dark:text-white text-3xl mb-4 sm:text-4xl lg:text-5xl">Riwayat project</h2>
                </div>
            </div>
                ...
                @foreach($projects as $project)
                    <div class="w-full px-4 lg:w-1/2 xl:w-1/3">
                        <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-10 dark:bg-slate-600">
                            <img src="/storage/{{ $project->image }}" alt="{{ $project->title }}" class="max-h-52 mx-auto">
                            <div class="py-8 px-6">
                                <a href="/project/{{ $project->slug }}" class="block  mb-3 font-semibold text-xl text-dark dark:text-white hover:text-primary truncate">
                                    <h3 class="">{{ $project->title }}</h3>
                                </a>
                                <a href="/project/{{ $project->slug }}" class="font-emdium text-sm text-white bg-primary py-2 px-4 rounded-lg hover:opacity-80">Baca selengkapnya</a>
                            </div>
                        </div>
                    </div>
                @endforeach
