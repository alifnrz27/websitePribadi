                ...
                @foreach($categories as $category)
                    <div class="w-full px-4 lg:w-1/2 xl:w-1/3">
                        <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-10 dark:bg-slate-600">
                            <img src="/storage/{{ $category->image }}" alt="{{ $category->title }}" class="max-h-52 mx-auto">
                            <div class="py-8 px-6">
                                ...
                                <a href="/category/{{ $category->slug }}" class="font-emdium text-sm text-white bg-primary py-2 px-4 rounded-lg hover:opacity-80">Baca selengkapnya</a>
                            </div>
                        </div>
                    </div>
                @endforeach
